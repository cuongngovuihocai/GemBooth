...
const maxRetries = 2 // Reduced retries for better UX on auth errors
  ...
  async ({model, prompt, inputFile, signal, apiKey}) => {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        // If it's an API key error, throw immediately, don't retry
        if (error.message && (error.message.includes('400') || error.message.includes('401') || error.message.includes('API key'))) {
           throw error;
        }
